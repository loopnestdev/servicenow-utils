...
# Change-tracking: set to "true" by configure_ functions when files are written or
# modified; enable_ functions use these to skip unnecessary service restarts on re-runs.
_SNAP_SVC_CHANGED="false"
_CLAMAV_SVC_CHANGED="false"
...
configure_tomcat() {
  ...
  # Guard: skip if already correct. Two-pass sed handles re-runs where a previous
  # run already added address=: pass 1 strips any existing address+port back to bare
  # port="0", pass 2 inserts the desired address+port. Both passes apply in order
  # on each line in a single sed invocation.
  if grep -q "address=\"127.0.0.1\" port=\"${SNAP_PORT}\"" "${server_xml}" 2>/dev/null; then
    log "Tomcat server.xml already configured for 127.0.0.1:${SNAP_PORT}, skipping."
  else
    sed -i \
      -e 's|<Connector address="[^"]*" port="[0-9]*" protocol="HTTP/1.1"|<Connector port="0" protocol="HTTP/1.1"|g' \
      -e "s|<Connector port=\"[0-9]*\" protocol=\"HTTP/1.1\"|<Connector address=\"127.0.0.1\" port=\"${SNAP_PORT}\" protocol=\"HTTP/1.1\"|" \
      "${server_xml}"
    log "Tomcat server.xml updated."
    _SNAP_SVC_CHANGED="true"
  fi

  # Write unit file only if content differs from what is already on disk.
  local unit_file="/etc/systemd/system/${SNAP_SVC}.service"
  local unit_tmp; unit_tmp=$(mktemp)
  cat > "${unit_tmp}" <<EOF
[Unit]
...
EOF
  if ! cmp -s "${unit_tmp}" "${unit_file}" 2>/dev/null; then
    cp "${unit_tmp}" "${unit_file}"
    _SNAP_SVC_CHANGED="true"
    log "Tomcat systemd unit written."
  else
    log "Tomcat systemd unit unchanged, skipping."
  fi
  rm -f "${unit_tmp}"
...
configure_clamd() {
  ...
  local _prev_md5; _prev_md5=$(md5sum "${conf_dir}/clamd.conf" 2>/dev/null | awk '{print $1}')
...
ExcludePath ^/mnt/
EOF
  local _new_md5; _new_md5=$(md5sum "${conf_dir}/clamd.conf" | awk '{print $1}')
  [ "${_prev_md5}" != "${_new_md5}" ] && _CLAMAV_SVC_CHANGED="true"

  # Write unit file only if content differs.
  local _clamd_unit_tmp; _clamd_unit_tmp=$(mktemp)
  cat > "${_clamd_unit_tmp}" <<EOF
[Unit]
...
WantedBy=multi-user.target
EOF
  if ! cmp -s "${_clamd_unit_tmp}" /etc/systemd/system/clamd.service 2>/dev/null; then
    cp "${_clamd_unit_tmp}" /etc/systemd/system/clamd.service
    _CLAMAV_SVC_CHANGED="true"
  fi
  rm -f "${_clamd_unit_tmp}"

  systemctl daemon-reload
  log "clamd.conf and systemd unit configured."
}
...
configure_freshclam() {
  ...
  local _prev_fc_md5; _prev_fc_md5=$(md5sum "${conf_dir}/freshclam.conf" 2>/dev/null | awk '{print $1}')
...
EOF
  local _new_fc_md5; _new_fc_md5=$(md5sum "${conf_dir}/freshclam.conf" | awk '{print $1}')
  [ "${_prev_fc_md5}" != "${_new_fc_md5}" ] && _CLAMAV_SVC_CHANGED="true"

  # Write freshclam override only if content differs.
  local override_dir="/etc/systemd/system/clamav-freshclam.service.d"
  mkdir -p "${override_dir}"
  local _fc_override_tmp; _fc_override_tmp=$(mktemp)
  cat > "${_fc_override_tmp}" <<EOF
[Service]
...
EOF
  if ! cmp -s "${_fc_override_tmp}" "${override_dir}/snap-config.conf" 2>/dev/null; then
    cp "${_fc_override_tmp}" "${override_dir}/snap-config.conf"
    _CLAMAV_SVC_CHANGED="true"
  fi
  rm -f "${_fc_override_tmp}"

  systemctl daemon-reload
  log "freshclam.conf and service override configured."
}
...
enable_tomcat() {
  ...
  if [ "${_SNAP_SVC_CHANGED}" = "true" ] || ! systemctl is-active --quiet "${SNAP_SVC}" 2>/dev/null; then
    systemctl restart "${SNAP_SVC}"
    log "${SNAP_SVC} service started."
  else
    log "${SNAP_SVC} already running and config unchanged — skipping restart."
  fi
}
...
enable_clamav() {
  ...
  if [ "${SKIP_FRESHCLAM}" = "true" ]; then
    ...
    if [ "${_CLAMAV_SVC_CHANGED}" = "true" ] || ! systemctl is-active --quiet clamav-freshclam 2>/dev/null; then
      systemctl restart clamav-freshclam
    else
      log "clamav-freshclam already running and config unchanged — skipping restart."
    fi
  ...
  if [ "${_CLAMAV_SVC_CHANGED}" = "true" ] || ! systemctl is-active --quiet clamd 2>/dev/null; then
    systemctl restart clamd
    log "ClamAV services started."
  else
    log "clamd already running and config unchanged — skipping restart."
  fi
}
